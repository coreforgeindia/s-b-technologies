...
import { products, stats, projects, faqs, industries } from "@/data/content";
import heroBg from "@/assets/hero-bg.png";
import { AnimatedCounter } from "@/components/animated-counter";
import { useState } from "react";

// Image URLs for Gallery Preview (using content assets)
...
export default function Home() {
  ...
  const previewGallery = [
    { url: coreConstructionImg, caption: "SMT Assembly Line" },
    { url: tankFabricationImg, caption: "PCB Inspection" },
    { url: terminationsImg, caption: "Reflow Soldering" }
  ];
      ...
      >
        ...
        <div className="max-w-[1280px] w-full relative z-10 px-4 md:px-8 mx-auto">
          ...
          >
            ...
            >
              S.B. TECHNOLOGIES <br />
              <span className="text-primary text-3xl sm:text-4xl md:text-5xl">ELECTRONICS MANUFACTURING SERVICES</span>
            </motion.h1>
        ...
        <div className="max-w-[1280px] mx-auto px-4 md:px-8">
          <div className="flex flex-col lg:flex-row gap-16 items-center">
            <div className="lg:w-1/2 space-y-6">
              <span className="text-primary font-bold text-sm uppercase tracking-wider">Trusted EMS Partner Since 1995</span>
              <h2 className="text-3xl md:text-4xl font-bold text-navy">Who We Are</h2>
              <div className="h-1 w-20 bg-primary"></div>
              <p className="text-zinc-600 text-lg leading-relaxed">
                S.B. TECHNOLOGIES is a Bangalore-based Electronics Manufacturing Services (EMS) provider offering PCB manufacturing, automated reflow soldering, through-hole and SMD technology assembly, and PCB CAD/CAM design services to clients across the globe.
              </p>
      ...
      <section className="py-24 bg-zinc-50 border-y border-zinc-150">
        <div className="max-w-[1280px] mx-auto px-4 md:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-navy mt-2 mb-4">Why Choose S.B. Technologies</h2>
              ...
              <div key={product.id} className="bg-white rounded-2xl border border-zinc-150 overflow-hidden shadow-sm hover:shadow-md hover:border-primary/45 transition-all group flex flex-col justify-between">
                <div>
                  <div className="h-44 bg-zinc-950 p-6 flex items-center justify-center border-b-2 border-primary">
                    <img src={product.image} alt={product.title} className="h-full w-full object-contain group-hover:scale-105 transition-transform duration-300" />
                  </div>
        ...
        <div className="max-w-[1280px] mx-auto px-4 md:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {previewGallery.map((gal, i) => (
              <div key={i} className="h-64 rounded-2xl overflow-hidden relative group border border-zinc-150 shadow-sm">
                <img src={gal.url} alt={gal.caption} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                <div className="absolute inset-0 bg-gradient-to-t from-zinc-950/70 to-transparent flex items-end p-6">
                  <h3 className="text-white font-bold text-base md:text-lg">{gal.caption}</h3>
                </div>
              </div>
            ))}
          </div>
